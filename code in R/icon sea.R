...
iconSea <- makeIcon(iconUrl = imageUrl, iconWidth = 50, iconHeight = 50)
...
map <- leaflet() %>%
  ...
  addMarkers(lng = -73.9857, lat = 40.7484, icon = iconSea) # add marker with custom icon
...
map <- htmlwidgets::onRender(map, "
  function(el, x) {
    this.on('zoomend', function() {
      ...
      var newIconSize = [40, 28];  // Change to new size for your marker
