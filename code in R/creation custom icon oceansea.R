...
imageUrl <- "oceansea.png"

# Create a custom icon
iconOceanSea <- makeIcon(iconUrl = imageUrl, iconWidth = 50, iconHeight = 50)
...
map <- leaflet() %>%
  ...
  addMarkers(lng = -73.9857, lat = 40.7484, icon = iconOceanSea) # add marker with custom icon
...
map <- htmlwidgets::onRender(map, "
  function(el, x) {
    this.on('zoomend', function() {
      ...
      var newIconSize = [35, 35];  // Change to new size for your marker
